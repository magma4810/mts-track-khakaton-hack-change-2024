...
export function DropdownMenu({ children, ...props }) {
  ...
  useEffect(() => {
    const cityes = [
      ...new Set(
        informationAboutUsers.map((city) => city.address.split(",")[0]),
      ),
    ];
    setCityes(cityes);
    const position = [
      ...new Set(informationAboutUsers.map((el) => el.position.name)),
    ];
    setPosition(position);
    const role = [...new Set(informationAboutUsers.map((el) => el.role.name))];
    setRole(role);
    const division = [
      ...new Set(informationAboutUsers.map((el) => el.division.name)),
    ];
  ...
  return (
    <>
      <button
        ...
        onClick={() => props.toggleDropdown(children)}
      >
        {children}
      </button>
      {props.active && (
        <ul className="dropdown-menu">
          {children === "Город" &&
            cityes.map((el) => (
              <MenuElement
                key={el}
                checked={checkedItems[el] || false}
                setСhecked={() => handleChange(el)}
              >
                {el}
              </MenuElement>
            ))}
